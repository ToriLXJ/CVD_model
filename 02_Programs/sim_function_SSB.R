...
run_sim <- function(s, intervention) {
  ...
  for (t in 1:n.cycle) {
    ...
    p.H.2.DM <- RR_diff_total_DM*as.numeric(sim_out_t[,"DM_prob"])  ##1个  -RR；看原模型的概率值对比
    ...
    p.sub_Stroke.2.Stroke_No_DM <- as.numeric(ifelse(as.numeric(sim_out_t[,"DM"]) ==1, 0, 1 - p.sub_Stroke.2.death))
...
=======
=======
>>>>>>> 9cf0a0a8050d18018c9f460bbbf7cea0efba11a5
#Updating Disease history status
update_risk_factor <- function(sim_out_t, risk_factor, predictor, s) {
  # Updates risk factor values from time t to time t+1
  risk_factor_vals <- case_when(
