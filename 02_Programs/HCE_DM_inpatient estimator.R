...
HCE_DM_inpatient_parameters <- read.csv("01_Input/HCE_DM_Inpatient Estimation Parameters.csv", row.names = 1,encoding = 'UTF-8')

calc_HCE_DM_inpatient <- function(raw.input.data, params) {
  # Input: CHNS data for analysis and HCE parameters
  names <- c(rownames(HCE_DM_inpatient_parameters))
  ...
  HCE_DM_inpatient_input<-raw.input.data[c("Subject_ID",names)]
  
  
  ############################################
  # If value is missing, place 0
  ############################################
  HCE_DM_inpatient_input[is.na(HCE_DM_inpatient_input)] <- 0
  
  HCE_DM_inpatient_baseline_mean <- 1547.94 # for male,Age under44
  HCE_DM_inpatient_baseline_SE <- 309.6
  

  HCE_DM_inpatient_input$HCE_DM_inpatient_predict<- HCE_DM_inpatient_baseline_mean + (as.matrix(HCE_DM_inpatient_input[2:8]) %*% as.matrix(params)) 

  return(HCE_DM_inpatient_input$HCE_DM_inpatient_predict)
}
